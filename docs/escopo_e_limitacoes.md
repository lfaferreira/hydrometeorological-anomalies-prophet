...
> regra de reset da de-acumulação, polígono da RMR, estatísticas espaciais e
...
| 4 | Comparação com a APAC incompatível por construção (limiar diário retrospectivo vs. observação prospectiva em 3h/4 postos; nível "alerta" ≥100mm inalcançável na média areal, cujo máximo na série reconstruída na Etapa 2 é 91,209160mm em 2022-05-28) | Etapa 7 | Pendente |
...
| 7d | [Descoberto na Etapa 2, na revisão final] A de-acumulação identificava o reset do acumulador pelo SINAL do `diff`. Como `tp` é float32, ocorrem `diff`s negativos minúsculos por arredondamento dentro de um mesmo ciclo — e a regra os tratava como reset, publicando o acumulado inteiro do ciclo como se fosse o incremento de uma hora. Corrigido para identificar o reset pela hora fixa e conhecida do ciclo do ERA5-Land (`01:00 UTC`), verificada nos 72 arquivos brutos: o maior `diff` negativo fora dessa hora em toda a série vale 4,76e-08 m (4,8e-05 mm). Impacto na série publicada anteriormente: 865 dos 2.192 dias (39,5%) estavam inflados, mediana +10,2% e máximo +526,6%; volume total de 6 anos caiu 3,4% (6.727,6mm → 6.497,7mm). O máximo da série (2022-05-28) não foi afetado. | Etapa 2 | Resolvido na Etapa 2 |
...
| 7c | [Descoberto na Etapa 2] O deslocamento combinado de -4h (correção de rótulo de janela + fuso `America/Recife`) sobre um dataset bruto que vai de `2020-01-01 00:00 UTC` a `2025-12-31 23:00 UTC` cria dias de calendário local parcialmente cobertos nas duas pontas. **Início — resolvido:** o primeiro passo bruto (`00:00 UTC`) não é início de ciclo de acumulação, e sim a cauda do ciclo de 2019-12-31, que não foi baixado; seu incremento é desconhecido e passou a ser marcado como NaN em vez de chutado, o que faz o `sum(skipna=False)` descartar o dia local `2019-12-31` (que teria só 4 das 24 horas). A série agora começa limpa em `2020-01-01` com **2192 linhas** (antes 2193, com uma primeira linha `2019-12-31` de 1,809074mm que não era um total diário). **Fim — permanece:** o último dia local (`2025-12-31`, 1,522995mm) tem 20 das 24 horas locais; as 4 restantes exigiriam os passos brutos de `2026-01-01 00:00–03:00 UTC`, fora da janela baixada. Aqui não há ambiguidade de de-acumulação — todos os incrementos presentes estão corretos —, apenas cobertura incompleta: é um subtotal real, não um total diário, e não deve ser lido como tal. | Etapa 2 (início resolvido); disposição do dia final (manter vs. recortar) deferida à Etapa 3 | Início resolvido; dia final documentado — decisão de manter ou recortar pendente antes do reajuste do Prophet |
