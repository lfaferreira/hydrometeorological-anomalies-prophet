...
| 7c | [Descoberto na Etapa 2] `serie_prophet_rmr_2020_2025.csv` tem 2193 linhas, não 2192: o deslocamento combinado de -4h (correção de rótulo de janela + fuso `America/Recife`) aplicado a um dataset bruto que começa e termina exatamente em fronteiras de dia UTC produz dois dias de calendário local com cobertura parcial nas pontas da série — `2019-12-31` com apenas 4 das 24 horas locais (00h–19h ausentes; total 1,809074mm, só ~4h de chuva real) e `2025-12-31` com 20 das 24 horas (20h–23h ausentes; as 4 horas que faltam exigiriam dados de 2026, inexistentes). `_daily_pixel_totals` soma com `skipna=False`, que descarta dias totalmente ausentes mas não sinaliza dias parcialmente cobertos — os dois valores de borda são reais, mas fisicamente incompletos, e não devem ser lidos como "totais diários completos" sem essa ressalva. | Etapa 2 (detectado); disposição (manter vs. recortar) deferida à Etapa 3 | Documentado — decisão de manter ou recortar os dias de borda pendente antes do reajuste do Prophet |
